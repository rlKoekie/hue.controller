...
def run():
    ...
    while not monitor.abortRequested():
        ...
        if debug == "true":
            player.loglevel = xbmc.LOGINFO
        else:
            player.loglevel = xbmc.LOGDEBUG
        # xbmc.log("%s lastplayingtype= %s lightswereon= %s loglevel=%s" %( time.time(), player.lastplayingtype, player.lightswereon, player.loglevel), level=xbmc.LOGINFO)
        # HueControllerADDON = xbmcaddon.Addon(id='plugin.program.hue.controller')
